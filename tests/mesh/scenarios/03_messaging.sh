...
ALPHA_IDHASH=$(styrene --socket "$ALPHA_SOCK" identity 2>&1 | awk '/hash/ && !/dest|lxmf/ {print $2; exit}')
BETA_DEST=$(styrene --socket "$BETA_SOCK" identity 2>&1 | grep "lxmf" | awk '{print $2}')
GAMMA_DEST=$(styrene --socket "$GAMMA_SOCK" identity 2>&1 | grep "lxmf" | awk '{print $2}')
...
if [ -z "$BETA_DEST" ] || [ -z "$ALPHA_DEST" ]; then
    ...
    if ! wait_for_peer "$ALPHA_SOCK" beta 60; then
