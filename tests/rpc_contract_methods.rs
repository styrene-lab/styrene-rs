...
fn message_and_announce_methods_contract() {
    ...
    commands_message::run_announce(
        &ctx,
        &AnnounceCommand {
            action: AnnounceAction::Now,
        },
    )
    .unwrap();
...
fn identity_resolution_contract_uses_status_fallback() {
    ...
    assert_eq!(
        observed,
        vec!["daemon_status_ex", "status", "send_message_v2"]
    );
...
fn propagation_contract_methods() {
    ...
    commands_propagation::run(
        &ctx,
        &PropagationCommand {
            action: PropagationAction::Status,
        },
    )
    .unwrap();
    commands_propagation::run(
    ...
    commands_propagation::run(
        &ctx,
        &PropagationCommand {
            action: PropagationAction::Fetch {
                transient_id: "tx-1".into(),
            },
        },
    )
    .unwrap();
...
fn spawn_scripted_rpc_server(
    ...
    let worker = thread::spawn(move || {
        ...
        assert_eq!(
            idx,
            expected.len(),
            "did not observe all expected rpc calls"
        );
        observed
    });
...
fn assert_params(params: &Option<Value>, required_keys: &[&str], method: &str) {
    if required_keys.is_empty() {
        assert!(
            params.is_none(),
            "method '{}' expected no params but got {:?}",
            method,
            params
        );
        return;
