...
- [x] 2.1 Add failing `MobileNode::poll_hub` tests for accepted, durable duplicate, decode rejection, storage failure, acknowledgement failure, and mixed partial outcomes
- [x] 2.2 Replace unconditional hub deletion with per-item durable acknowledgement eligibility or delegate the helper to standard propagation synchronization
- [x] 2.3 Add bounded Unicode-safe preview tests at ASCII, multibyte scalar, combining-mark, empty, and over-limit boundaries
- [x] 2.4 Expose typed per-item and remote acknowledgement outcomes without weakening standard-propagation durable-before-ACK behavior

## 3. Identity Custody And Metadata
<!-- specs: mobile-backend-identity -->

- [x] 3.1 Add failing target and feature-matrix tests proving Keychain, Android Keystore, and encrypted-file selection never falls back to plaintext
...
- [x] 4.1 Add a typed offline-ready runtime projection independent from transport and bearer state, with boot, shutdown, and no-interface tests
- [x] 4.2 Add bounded boot-stage and recovery failures and verify failed partial composition releases workers, listeners, and stores
- [x] 4.3 Populate current generation and typed failure reasons in interface, capability, and storage observations
- [x] 4.4 Add isolated child-process forced-termination tests for committed state, interrupted work, migration, exactly-once restoration, bounded deadlines, and unconditional cleanup
- [x] 4.5 Expose storage schema, open, recovery, commit, and degraded status without private paths

## 5. Conversation And Contact Projection
<!-- specs: mobile-backend-messaging -->

- [x] 5.1 Add a durable conversation-shell schema and idempotent typed start-conversation operation for canonical delivery destinations
- [x] 5.2 Include empty shells in bounded conversation queries without fabricated preview, timestamp, unread, route, or connectivity state
- [x] 5.3 Resolve display identity by contact alias, canonical announce name, then bounded destination abbreviation
- [x] 5.4 Emit typed conversation invalidation after contact mutation and test alias edit, removal, restart, and unrelated-peer isolation

## 6. Attempt Route And Bearer Evidence
<!-- specs: mobile-backend-messaging, mobile-backend-runtime -->

- [x] 6.1 Define immutable generation-scoped route and interface observation DTOs that can be referenced by one message attempt
...
- [x] 7.1 Add a bounded chronological diagnostic ring with stable sequence, source, stage, severity, time, generation, safe correlation, and truncation metadata
- [x] 7.2 Add an allowlisted redacted export DTO and canonical serialization independent from generic `Debug`
- [x] 7.3 Add mutation tests for message content, title, wire bytes, attachments, keys, credentials, tokens, passphrases, private paths, encoded variants, capacity, and deterministic digest
- [x] 7.4 Expose bounded export bytes and metadata while leaving file save and platform sharing to `styrene-ui`
