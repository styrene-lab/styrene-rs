...
- [ ] 1.2 Add failing Rust serialization and reducer-contract tests for every fixture state before changing production DTOs
- [ ] 1.3 Add failing Swift and Kotlin fixture projection tests using the same fixture identifiers and required accessibility identifiers
...
- [ ] 2.1 Add failing `MobileNode` tests for persisted identity, hostname and IPv4 TCP clients, refused endpoints, bounded reconnect, and one-node ownership
- [ ] 2.2 Extend mobile status and UniFFI contracts with typed connection phase, endpoint, generation, failure, and independent bearer observations
- [ ] 2.3 Add failing `StyreneNodeModelTests` and `MobileNodeStateHolderTest` cases for cold restoration, reconnect, stale completion rejection, and TCP operation without an RNode
- [ ] 2.4 Implement persisted endpoint editing, automatic boot, reconnect presentation, and recoverable failure on both hosts
...
- [ ] 3.3 Add failing Swift and Kotlin tests for repeated announces, stale generation events, empty live directories, and local announce outcomes
...
- [ ] 4.2 Extend mobile DTOs and methods for drafts, requested and actual method, attempt, propagation upload, receipt evidence, retry, and typed failure
- [ ] 4.3 Add failing Swift and Kotlin tests for draft revision races, empty live state, queued versus delivered rendering, inbound unread behavior, duplicate evidence, retry, and restart restoration
- [ ] 4.4 Implement native conversation, composer, history, retry, and delivery-detail projections using backend-owned state
...
- [ ] 5.2 Expose standard propagation discovery, selection, readiness, upload, manual sync, automatic-sync policy, progress, and terminal observations through mobile DTOs and UniFFI
...
- [ ] 5.4 Add failing Swift and Kotlin tests for selection persistence, stale node metadata, manual sync, automatic sync disclosure, upload versus delivery, progress, repeat sync, and recoverable failure
...
- [ ] 6.1 Add failing host tests proving TCP remains operational when Bluetooth or USB is unavailable, denied, interrupted, or unverified
- [ ] 6.2 Present TCP, Bluetooth RNode, and Android USB as independent backend-confirmed bearer states
...
- [ ] 7.1 Run the shared state corpus in Swift unit and UI tests and Android unit and Compose tests
...
- [ ] 8.2 Build, install, cold-launch, and inspect fatal logs for iOS and Android release candidates
