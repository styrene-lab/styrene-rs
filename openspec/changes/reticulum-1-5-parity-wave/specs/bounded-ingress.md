...
Path-request state must preserve canonical limits: 16,000 replay tags per count-rotated generation, one 120-second in-flight gate per destination, at most one waiter per registered interface for a destination, and at most 32 pending discovery transmissions.
...
And a 120-second in-flight expiry or count-based tag-generation rotation releases the corresponding state
