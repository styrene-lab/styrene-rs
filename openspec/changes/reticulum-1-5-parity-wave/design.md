...
Task 7 consumes Task 6 bitrate metadata but does not define or mutate it. Task 7 owns optional positive hardware MTU metadata, link-MTU enable/disable policy, request signaling, route-minimum clamping, proof construction and validation, and packet/channel/resource payload derivation. Link MTU discovery defaults on and can be disabled globally or per composition. Initiators always signal the default mode and either the route MTU when discovery is enabled and supported or the canonical base MTU otherwise. Each forwarding hop clamps to the minimum supported MTU or removes signaling when an interface cannot participate. Destinations always authenticate the confirmed mode and MTU in the proof, using the canonical base MTU after stripped legacy requests. Packet MDU, channel MDU, and resource SDU derive from the negotiated link MTU.
