...
A separate in-flight gate is keyed only by destination, permits exactly one entry per destination, and expires after canonical Reticulum 1.5.1 `PATH_REQUEST_GATE_TIMEOUT` of 45 seconds. Canonical Python does not impose a fixed total in-flight-map cardinality, so this plan does not invent one; its measurable bound is one entry per distinct destination admitted during the 45-second lifetime. The first eligible request starts discovery; later valid requests for the same destination batch into it and do not emit another recursive request.

Waiters contain at most one entry for each currently registered requesting interface. Duplicate requests from one interface do not grow waiter state, and detached interfaces are removed or ignored before response. Thus the exact waiter bound for one destination is the current registered-interface count, not a new numeric constant. The pending discovery transmit queue has canonical maximum 32; when full, a new pending discovery item is not enqueued and the drop is observable. Discovery waiters expire with their discovery request deadline while the independent destination gate remains active for its canonical lifetime. A later unrestricted request under that gate recreates waiter state without emitting another recursive request.

Canonical 1.5.1 has no separate fixed-cardinality overflow branch for the in-flight map or per-destination waiter list, so this wave does not invent one or reject otherwise valid requests at an arbitrary count. Their exact bounds are identity and lifetime bounds: one in-flight entry per destination for 45 seconds and one waiter per currently registered interface. Static overflow behavior applies only to the four ingress queues and the 32-entry pending discovery transmit queue.
