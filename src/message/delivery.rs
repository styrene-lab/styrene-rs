    ...
    if matches!(method, TransportMethod::Opportunistic) {
        ...
        if content_size > limit {
            ...
            return Ok(DeliveryDecision {
                method,
                representation: MessageMethod::Packet,
            });
        }
    }
    ...
    match method {
        ...
        TransportMethod::Paper => {
            if content_size <= PAPER_MDU {
                Ok(DeliveryDecision {
                    method,
                    representation: MessageMethod::Paper,
                })
            } else {
                Err(LxmfError::Encode(
                    "paper delivery content exceeds paper MDU".into(),
                ))
            }
        }
        TransportMethod::Opportunistic => Err(LxmfError::Encode(
            "opportunistic delivery could not be resolved".into(),
        )),
    }
}
