...
fn parses_profile_init_command() {
    ...
    match cli.command {
        Command::Profile(ProfileCommand { action: ProfileAction::Init { name, managed, rpc } }) => {
            assert_eq!(name, "demo");
            assert!(managed);
...
fn parses_message_send_command() {
    ...
    match cli.command {
        Command::Message(MessageCommand { action: MessageAction::Send(args) }) => {
...
fn parses_profile_set_command() {
    ...
    match cli.command {
        Command::Profile(ProfileCommand {
            action: ProfileAction::Set { display_name, clear_display_name, name },
        }) => {
...
fn parses_peer_query_and_exact_flags() {
    ...
    match list_cli.command {
        Command::Peer(PeerCommand { action: PeerAction::List { query, limit } }) => {
    ...
    match show_cli.command {
        Command::Peer(PeerCommand { action: PeerAction::Show { selector, exact } }) => {
...
fn parses_contact_commands() {
    ...
    match add_cli.command {
        Command::Contact(ContactCommand { action: ContactAction::Add(args) }) => {
    ...
    match list_cli.command {
        Command::Contact(ContactCommand { action: ContactAction::List { query, limit } }) => {
