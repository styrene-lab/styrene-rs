...
fn contact_add_show_remove_roundtrip() {
    ...
    let ctx = RuntimeContext {
        cli: Cli {
            ...
            command: Command::Contact(ContactCommand {
                action: ContactAction::List { query: None, limit: None },
            }),
        },
    ...
    commands_contact::run(
        &ctx,
        &ContactCommand { action: ContactAction::Show { selector: "alice".into(), exact: false } },
    )
    .unwrap();

    commands_contact::run(
        &ctx,
        &ContactCommand { action: ContactAction::Remove { selector: "alice".into(), exact: true } },
    )
    .unwrap();
