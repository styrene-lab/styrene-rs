...
impl Lane {
    fn all() -> [Self; 4] {
        [Self::PythonToPython, Self::PythonToRust, Self::RustToPython, Self::RustToRust]
    }
...
impl RustContractAdapter {
    fn new() -> Self {
        Self { router: Router::with_adapter(Adapter::new()), ..Self::default() }
    }
}
...
impl ContractAdapter for RustContractAdapter {
    ...
    fn send(&mut self, message: ContractMessageInput) -> String {
        let payload =
            Payload::new(1_700_000_000.0, Some(message.content), message.title, None, None);
        ...
        self.statuses.insert(
            handle.clone(),
            ContractStatus { state: ContractState::Queued, progress: 0, detail: None },
        );
        handle
    }
    ...
    fn cancel(&mut self, handle: &str) -> bool {
        ...
        if cancelled {
            self.statuses.insert(
                handle.to_string(),
                ContractStatus { state: ContractState::Cancelled, progress: 0, detail: None },
            );
        }
        cancelled
    }
    ...
    fn tick(&mut self, max_outbound: usize) {
        let results = self.router.handle_outbound(max_outbound).expect("outbound processing");
        self.router.jobs();
        ...
        for result in results {
            ...
            let progress = self.router.outbound_progress(&result.message_id).unwrap_or(0);
            self.statuses.insert(handle, ContractStatus { state, progress, detail });
        }
    }
...
fn run_fixture(fixture: &ContractFixture, lane: Lane) {
    ...
    adapter.configure_policy(
        ...
        if fixture.allow_destination.unwrap_or(false) { Some(destination) } else { None },
    );
    ...
    assert!(!handle.is_empty(), "scenario {}: adapter must return a stable handle", fixture.id);

    if let Some(expected) = fixture.expected_state_before_tick.as_deref() {
        assert_state(fixture, &handle, adapter.status(&handle), expected, "before tick");
    }
    ...
    if fixture.cancel_before_tick.unwrap_or(false) {
        ...
        if let Some(expected) = fixture.expect_cancel_result {
            assert_eq!(cancelled, expected, "scenario {}: unexpected cancel() result", fixture.id);
        }
    }
    ...
    if let Some(expected) = fixture.expected_state_after_tick.as_deref() {
        assert_state(fixture, &handle, adapter.status(&handle), expected, "after scenario actions");
    }
}
...
fn assert_state(
    ...
    let actual = actual.unwrap_or_else(|| {
        panic!("scenario {} [{}]: missing status for handle {}", fixture.id, phase, handle)
    });
    assert_eq!(
