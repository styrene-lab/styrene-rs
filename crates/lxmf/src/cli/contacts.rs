...
pub fn load_contacts(profile_name: &str) -> Result<Vec<ContactEntry>> {
    ...
    let mut contacts =
        parsed.contacts.into_iter().map(validate_contact).collect::<Result<Vec<_>>>()?;
...
pub fn save_contacts(profile_name: &str, contacts: &[ContactEntry]) -> Result<()> {
    ...
    let mut normalized =
        contacts.iter().cloned().map(validate_contact).collect::<Result<Vec<_>>>()?;
    dedupe_contacts(&mut normalized);
    sort_contacts(&mut normalized);

    let encoded = toml::to_string_pretty(&ContactsFile { contacts: normalized })
...
pub fn import_contacts_json(profile_name: &str, src: &Path, replace: bool) -> Result<usize> {
    ...
    let mut imported = imported.into_iter().map(validate_contact).collect::<Result<Vec<_>>>()?;
...
pub fn upsert_contact(contacts: &mut Vec<ContactEntry>, entry: ContactEntry) {
    ...
    if let Some(existing) =
        contacts.iter_mut().find(|contact| contact.alias.eq_ignore_ascii_case(&entry.alias))
    {
        *existing = entry.clone();
        updated = true;
    } else if let Some(existing) =
        contacts.iter_mut().find(|contact| contact.hash.eq_ignore_ascii_case(&entry.hash))
    {
        *existing = entry.clone();
        updated = true;
    }
...
pub fn find_contact_by_hash<'a>(
    ...
    contacts.iter().find(|entry| entry.hash.eq_ignore_ascii_case(hash))
}
...
pub fn select_contacts<'a>(
    ...
    ranked.sort_by(|(score_a, entry_a), (score_b, entry_b)| {
        score_a.cmp(score_b).then_with(|| compare_contacts(entry_a, entry_b))
    });
...
pub fn filter_contacts(
    ...
    let mut result = if let Some(query) = query.and_then(trimmed_nonempty) {
        select_contacts(contacts, query, false).into_iter().cloned().collect::<Vec<_>>()
    } else {
        contacts.to_vec()
    };
...
fn normalize_hash(value: &str) -> Result<String> {
    ...
    if trimmed.len() != 32 {
        return Err(anyhow!("contact hash must be a 32-character hex destination hash"));
    }
