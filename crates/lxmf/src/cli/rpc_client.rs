...
fn read_response_body(response: ureq::Response) -> Result<Vec<u8>> {
    let content_length =
        response.header("Content-Length").and_then(|value| value.parse::<usize>().ok());
    let mut reader = response.into_reader();

    if let Some(length) = content_length {
        let mut bytes = vec![0u8; length];
        if length > 0 {
            reader.read_exact(&mut bytes).context("failed to read content-length bytes")?;
        }
        return Ok(bytes);
    }

    let mut bytes = Vec::new();
    reader.read_to_end(&mut bytes).context("failed to read response body")?;
    Ok(bytes)
}
