...
impl BleBackend for MacOsBleBackend {
    ...
    fn scan(&mut self, settings: &BleRuntimeSettings) -> Result<(), BleBackendError> {
            ...
            .is_some_and(|adapter| adapter.eq_ignore_ascii_case("disabled"))
        {
            return Err(BleBackendError::terminal(
                "macOS BLE adapter is disabled by configuration",
            ));
        }
        Ok(())
    }
    ...
    fn connect(&mut self, settings: &BleRuntimeSettings) -> Result<(), BleBackendError> {
        if settings.peripheral_id == "00:00:00:00:00:00" {
            return Err(BleBackendError::terminal(
                "macOS BLE peripheral_id is invalid (all zeros)",
            ));
        }
        Ok(())
    }
    ...
    fn write_probe(
        ...
        if !synthetic_probe_enabled() {
            return Err(BleBackendError::terminal(
                "macOS BLE probe requires platform GATT I/O; synthetic loopback disabled (set LXMF_BLE_SYNTHETIC_PROBE=1 to bypass)",
            ));
        }
    ...
    fn read_probe_notification(
        ...
        if !synthetic_probe_enabled() {
            return Err(BleBackendError::terminal(
                "macOS BLE probe notification unavailable without platform GATT I/O",
            ));
        }
...
pub(super) fn startup(
    ...
    let mut backend = MacOsBleBackend::default();
...
fn log_report(iface: &InterfaceConfig, settings: &BleRuntimeSettings, report: &BleLifecycleReport) {
    eprintln!(
        "[daemon] ble_gatt configured (macos backend) name={} adapter={} peripheral_id={} service_uuid={} write_char_uuid={} notify_char_uuid={} mtu={} scan_timeout_ms={} connect_timeout_ms={} reconnect_backoff_ms={} max_reconnect_backoff_ms={} attempts={} transitions={}",
