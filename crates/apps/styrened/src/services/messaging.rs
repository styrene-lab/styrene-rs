...
impl MessagingService {
    ...
    pub async fn send_chat(
        ...
        let delivery_result = Self::deliver(transport.as_ref(), dest_hash, &payload).await;
    ...
    pub fn get_message(&self, message_id: &str) -> Result<Option<MessageRecord>, std::io::Error> {
        self.store.lock().unwrap().get_message(message_id).map_err(std::io::Error::other)
    }
    ...
    pub fn list_messages(
        ...
        self.store.lock().unwrap().list_messages(limit, before_ts).map_err(std::io::Error::other)
    }

    /// Count message buckets (inbound, outbound).
    pub fn count_messages(&self) -> Result<(u64, u64), std::io::Error> {
        self.store.lock().unwrap().count_message_buckets().map_err(std::io::Error::other)
    }
    ...
    pub fn mark_read(&self, peer_hash: &str) -> Result<u64, std::io::Error> {
        self.store.lock().unwrap().mark_read(peer_hash).map_err(std::io::Error::other)
    }

    /// Delete all messages in a conversation with a peer.
    pub fn delete_conversation(&self, peer_hash: &str) -> Result<u64, std::io::Error> {
        self.store.lock().unwrap().delete_conversation(peer_hash).map_err(std::io::Error::other)
    }

    /// Delete a single message by ID.
    pub fn delete_message(&self, message_id: &str) -> Result<bool, std::io::Error> {
        self.store.lock().unwrap().delete_message(message_id).map_err(std::io::Error::other)
    }
    ...
    pub fn list_conversations(
        ...
        self.store.lock().unwrap().list_conversations(unread_only).map_err(std::io::Error::other)
    }
    ...
    pub fn remove_contact(&self, peer_hash: &str) -> Result<bool, std::io::Error> {
        self.store.lock().unwrap().remove_contact(peer_hash).map_err(std::io::Error::other)
    }
    ...
    pub fn list_contacts(
        ...
        self.store.lock().unwrap().list_contacts().map_err(std::io::Error::other)
    }
    ...
    pub fn track_receipt(&self, packet_hash: &str, message_id: &str) {
        self.receipt_map.lock().unwrap().insert(packet_hash.to_string(), message_id.to_string());
    }
    ...
    pub fn handle_receipt(&self, packet_hash: &str, status: &str) -> Result<bool, std::io::Error> {
    ...
    pub fn clear_messages(&self) -> Result<(), std::io::Error> {
        self.store.lock().unwrap().clear_messages().map_err(std::io::Error::other)
    }
