...
impl MeshTransport for MockTransport {
    async fn send_raw(
        ...
        self.record(MockCall::SendRaw { dest, data: data.to_vec() });
...
mod tests {
    ...
    async fn send_raw_records_call_and_returns_default() {
        ...
        assert!(
            matches!(&mock.calls()[0], MockCall::SendRaw { dest: d, data } if *d == dest && data == b"hello")
        );
    }
    ...
    async fn announce_records_call_with_data() {
        ...
        assert!(matches!(&mock.calls()[1], MockCall::Announce { app_data: None }));
    }
