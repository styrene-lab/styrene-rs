...
impl RpcDaemon {
    ...
    pub fn uptime_secs(&self) -> u64 {
        0 // TODO: track start time in RpcDaemon
    }

    /// Number of known peers from the in-memory peer cache.
    pub fn peer_count(&self) -> usize {
        self.peers.lock().expect("peers lock").len()
    }

    /// Propagation state snapshot for API responses.
    pub fn propagation_snapshot(&self) -> serde_json::Value {
        let state = self.propagation_state.lock().expect("propagation lock").clone();
        serde_json::json!({
