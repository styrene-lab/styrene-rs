...
pub fn handle_http_request_with_transport_auth(
    ...
    let response = (|| -> io::Result<Vec<u8>> {
        ...
        match (method.as_str(), path_only) {
            ...
            // ── REST API (hub operations) — all authenticated ─────────
            ("GET", "/api/status") => {
                if let Err(error) = daemon.authorize_http_request_with_transport(
                    &parsed_headers,
                    peer_ip.as_deref(),
                ...
                let body = serde_json::to_vec(&json!({
                    ...
                    "peer_count": daemon.peer_count(),
                    "propagation": daemon.propagation_snapshot(),
                }))
                .map_err(io::Error::other)?;
                Ok(build_json_response(StatusCode::Ok, &body))
            }
            ("GET", "/api/devices") => {
                ...
                let announces =
                    daemon.messages().list_announces(500, None, None).map_err(io::Error::other)?;
                let devices: Vec<serde_json::Value> = announces
                    .iter()
                    .map(|a| {
                        json!({
                            "peer": a.peer,
                    ...
                    .collect();
                let body =
                    serde_json::to_vec(&json!({ "devices": devices })).map_err(io::Error::other)?;
                Ok(build_json_response(StatusCode::Ok, &body))
            }
            ("GET", "/api/propagation") => {
                if let Err(error) = daemon.authorize_http_request_with_transport(
                ...
                let stats = daemon.propagation_stats();
                let body = serde_json::to_vec(&json!({
                    "propagation": daemon.propagation_snapshot(),
                    "stored_count": stats.0,
            ...
            ("GET", "/api/pages") => {
                ...
                // Page listing — requires AppContext which this handler doesn't have.
                // Return a stub for now; full implementation uses DaemonFacade.
                let body = serde_json::to_vec(&json!({
                    "pages": [],
                    "note": "page listing via /api/pages requires hub mode"
                }))
                .map_err(io::Error::other)?;
                Ok(build_json_response(StatusCode::Ok, &body))
            }
