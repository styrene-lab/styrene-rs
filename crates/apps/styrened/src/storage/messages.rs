...
impl MessagesStore {
    ...
    pub fn delete_message(&self, message_id: &str) -> rusqlite::Result<bool> {
        let count = self.conn.execute("DELETE FROM messages WHERE id = ?1", params![message_id])?;
        Ok(count > 0)
    }
    ...
    pub fn remove_contact(&self, peer_hash: &str) -> rusqlite::Result<bool> {
        let count =
            self.conn.execute("DELETE FROM contacts WHERE peer_hash = ?1", params![peer_hash])?;
        Ok(count > 0)
    }
