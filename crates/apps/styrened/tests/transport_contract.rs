...
use styrened::transport::mesh_transport::{MeshTransport, TransportError, TransportLifecycleEvent};
...
async fn mock_lifecycle_fanout_reaches_multiple_subscribers() {
    ...
    assert_eq!(rx1.recv().await.unwrap(), TransportLifecycleEvent::Reconnected);
    assert_eq!(rx2.recv().await.unwrap(), TransportLifecycleEvent::Reconnected);
}
