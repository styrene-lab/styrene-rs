...
pub fn run(ctx: &RuntimeContext, command: &StampCommand) -> Result<()> {
    match &command.action {
        StampAction::Target => {
            ...
            let target = policy.get("target_cost").and_then(|v| v.as_u64()).unwrap_or_default();
        ...
        StampAction::Set { target_cost, flexibility } => {
        ...
        StampAction::GenerateTicket { destination, ttl_secs } => {
