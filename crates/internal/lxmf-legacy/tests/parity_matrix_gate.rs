...
fn done_parity_items_require_behavior_tests() {
    ...
    for (id, item) in &items {
        match item.status.as_str() {
            "done" => {
                assert!(!item.tests.is_empty(), "done item must list tests for id={id}");
...
fn lxmf_module_map_and_compatibility_matrix_are_fully_done() {
    ...
    let compatibility_text =
        std::fs::read_to_string("../../../docs/contracts/compatibility-matrix.md").unwrap();
    for line in compatibility_text.lines() {
        ...
        let columns: Vec<_> = line.split('|').map(str::trim).collect();
        let status = columns.get(3).copied().unwrap_or_default();
        assert_eq!(status, "done", "compatibility matrix row must be done: {line}");
    }
}
