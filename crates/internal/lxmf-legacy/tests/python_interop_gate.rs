...
fn python_to_rust_wire_roundtrip() {
    ...
    let wire_b64 = parsed.get("wire_b64").and_then(serde_json::Value::as_str).expect("wire_b64");
    let envelope_b64 =
        parsed.get("envelope_b64").and_then(serde_json::Value::as_str).expect("envelope_b64");
    let expected_content = parsed
        ...
        .to_vec();
    let expected_title =
        parsed.get("title").and_then(serde_json::Value::as_str).expect("title").as_bytes().to_vec();

    let wire_bytes =
        base64::engine::general_purpose::STANDARD.decode(wire_b64).expect("valid base64 wire");
    ...
    assert_eq!(message.payload.content, Some(ByteBuf::from(expected_content)));
