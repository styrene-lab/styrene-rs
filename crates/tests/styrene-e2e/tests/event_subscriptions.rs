...
async fn device_event_emitted_on_announce() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-ev").tcp_server("127.0.0.1:0").build().await;
...
async fn message_event_emitted_on_delivery() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-msg-ev").tcp_server("127.0.0.1:0").build().await;
        ...
        let message_events: Vec<_> =
            events.iter().filter(|e| matches!(e, DaemonEvent::Message { .. })).collect();
...
async fn link_event_emitted_during_message_delivery() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-link-ev").tcp_server("127.0.0.1:0").build().await;
        ...
        let link_events: Vec<_> =
            events.iter().filter(|e| matches!(e, DaemonEvent::Link { .. })).collect();
        ...
        assert!(!active_events.is_empty(), "should have a Link event with status 'active'");

        // The link event should reference bob's delivery hash
        if let DaemonEvent::Link { event } = active_events[0] {
            assert!(!event.link_id.is_empty(), "link_id should be non-empty");
            assert!(!event.peer_hash.is_empty(), "peer_hash should be non-empty");
        }
    })
    .await;
}
...
async fn multiple_device_events_from_repeated_announces() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-multi-ev").tcp_server("127.0.0.1:0").build().await;
        ...
        let device_events: Vec<_> =
            events.iter().filter(|e| matches!(e, DaemonEvent::Device { .. })).collect();
