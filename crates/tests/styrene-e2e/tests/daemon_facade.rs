...
fn facade_for(node: &styrene_e2e::node::TestNode) -> DaemonFacade {
    // Grant Admin role to the test node's own identity so all capabilities are available
    let entry =
        styrene_rbac::RosterEntry::new(node.identity_hash.clone(), styrene_rbac::Role::Admin);
    node.app_context.policy().grant(entry, node.app_context.store()).expect("grant admin");
...
async fn query_status_reflects_connected_state() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-status").tcp_server("127.0.0.1:0").build().await;
...
async fn query_identity_returns_correct_hashes() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-id").tcp_server("127.0.0.1:0").build().await;
...
async fn set_identity_updates_display_name() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-set-id").tcp_server("127.0.0.1:0").build().await;

        let facade = facade_for(&alice);

        let changed =
            facade.set_identity(Some("Alice Node"), None, None).await.expect("set_identity");
        assert!(changed);
...
async fn contact_crud_lifecycle() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-contacts").tcp_server("127.0.0.1:0").build().await;
        ...
        // Update contact
        let updated =
            facade.set_contact(peer_hash, Some("Robert"), None).await.expect("update contact");
...
async fn conversation_operations_after_message_exchange() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-conv").tcp_server("127.0.0.1:0").build().await;
        ...
        let convos = bob_facade.query_conversations(false).await.expect("query_conversations");
        ...
        // Mark read
        let read_count = bob_facade.mark_read(&convos[0].peer_hash).await.expect("mark_read");
        assert!(read_count >= 1);

        // Search messages
        let results = bob_facade.search_messages("conv-test", None, 10).await.expect("search");
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].content, "conv-test-1");

        // Search with wrong query
        let no_results =
            bob_facade.search_messages("nonexistent", None, 10).await.expect("search empty");
        ...
        let convos = bob_facade.query_conversations(false).await.expect("query after delete");
...
async fn auto_reply_configuration() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-ar").tcp_server("127.0.0.1:0").build().await;
        ...
        // Enable
        facade.set_auto_reply("all", Some("I'm away"), Some(120)).await.expect("set");
        ...
        // Disable
        facade.set_auto_reply("disabled", None, None).await.expect("disable");
...
async fn query_devices_after_announce() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-dev").tcp_server("127.0.0.1:0").build().await;
        ...
        assert!(!devices.is_empty(), "should see at least one device after announce");
...
async fn search_peers_finds_by_name() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-search").tcp_server("127.0.0.1:0").build().await;
...
async fn block_and_unblock_peer() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-block").tcp_server("127.0.0.1:0").build().await;
        ...
        assert!(blocked.contains(&peer.to_string()), "peer should be in blocked list");
...
async fn bookmark_peer() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-bm").tcp_server("127.0.0.1:0").build().await;
        ...
        // Bookmark bob
        facade.bookmark_peer(&bob.delivery_hash).await.expect("bookmark");
        ...
        // Unbookmark
        facade.unbookmark_peer(&bob.delivery_hash).await.expect("unbookmark");
...
async fn query_path_info_for_known_peer() {
    with_timeout(async {
        let alice = TestNodeBuilder::new("alice-path").tcp_server("127.0.0.1:0").build().await;
