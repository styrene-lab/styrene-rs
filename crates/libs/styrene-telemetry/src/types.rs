...
use crate::records::{
    ...
    ShipPosition, WeatherObservation,
};
...
impl TelemetryType {
    ...
    pub fn category(self) -> &'static str {
        match self {
            ...
            Self::WeatherObservation | Self::AirQuality | Self::SkyQuality => "environmental",
            Self::SatellitePass
            | Self::SatelliteTelemetry
            | Self::SpectrumObservation
            | Self::SignalReport => "rf-intelligence",
            Self::ServiceAnnouncement | Self::ServiceQuery | Self::ServiceResponse => {
                "coordination"
            }
...
impl TelemetryBatch {
    ...
    pub fn new(origin: [u8; 16], timestamp: u64) -> Self {
        Self { version: 1, timestamp, origin, records: HVec::new() }
    }
...
mod tests {
    ...
    fn telemetry_type_roundtrip() {
        ...
        for t in types {
            let code = t.as_u16();
            assert_eq!(
                TelemetryType::from_u16(code),
                Some(t),
                "from_u16({code:#06x}) should return {:?}",
                t
            );
        }
    }
    ...
    fn unknown_record_type_code_preserved() {
        let r = TelemetryRecord::Unknown { type_code: 0xBEEF, raw_bytes: HVec::new() };
    ...
    fn of_type_filters_correctly() {
        let mut b = TelemetryBatch::new([0u8; 16], 0);
        b.push(sample_aircraft()).unwrap();
        b.push(TelemetryRecord::Unknown { type_code: 0x9999, raw_bytes: HVec::new() }).unwrap();
        let aircraft: heapless::Vec<_, MAX_BATCH_RECORDS> =
            b.of_type(TelemetryType::AircraftPosition as u16).cloned().collect();
